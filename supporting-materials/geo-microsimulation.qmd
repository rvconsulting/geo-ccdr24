...
pp_microsim6 <- pp_microsim5 |>
  ...
  mutate(
    ...
    SAM3_job2 = 
      if_else(
        !is.na(Q13) & is.na(SAM3_job2), 
        other_sector_job1, 
        NA)
      ) |>
  # Drop the temporary 'other_sector' variable
  select(-other_sector_job1) |>
  ungroup()
```
...
pp_microsim7 <- pp_microsim6 |>
  ...
  mutate(
    lmarket = case_when(
      (!is.na(Q7) | !is.na(Q13))   ~ as.numeric(SAM3_job1),
      (!is.na(Q7) | !is.na(Q13))   ~ as.numeric(SAM3_job1),
      (!is.na(Q7) | !is.na(Q13))   ~ as.numeric(SAM3_job1),
      (is.na(Q7) & is.na(Q13)) & age > 14 ~ 4, # Unemployed
      (is.na(Q7) & is.na(Q13)) & age < 15 ~ 5,  # OLF
      .default = 10
    )
  )
```
