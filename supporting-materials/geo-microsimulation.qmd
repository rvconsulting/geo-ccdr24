...
We add economic targets from lmarket, by combining totals from the survey and growth rates by share from the macro file so that shares do not have wild changes (since the shares of lmarket are so different between the macro file and the actual survey) 
...
for (sc in scenarios) {
  for (yr in analysis_years) {
    ...
    # 2. Get labor market targets from your newly computed table
    ...
    # 4. Save using naming convention
  ...
  for (yr in analysis_years) {
    ...
    wages_sector <- pp_microsim12 |>
      ...
      left_join(
        ...
        by = character()  # join by recycling since it’s a single row
      ) |>
      mutate(
...
Create household-level total monthly labor income (mli_*) and multiplier coefficients (mli_coef_*) for each scenario.

These will allow us to rescale:
...
ca_microsim01 <- poverty |> 
  ...
  mutate(
    poor_2023 = if_else(cpsc == 100, 1, 0),
...
for (coef in coef_cols) {
  ...
  ca_microsim01[[poor_col]] <-
    if_else(ca_microsim01[[ae_col]] < ca_microsim01$pline, 1, 0)
}
...
#   mutate(
#     agr_income_2030_labprod = 
#       agr_income_2023 *
#       (100 + labprod_Agriculture_2030)/100,
#     agr_income_2050_labprod = 
#       agr_income_2023 *
#       (100 + labprod_Agriculture_2050)/100) |> 
#   mutate(
...
#                    self_employment_income_2030_baseline_labprod,
#                    agr_income_2030_labprod), na.rm = TRUE),
#     
...
```{r}
# Initial year
poor_2023 <- ca_microsim01 |>
  group_by(poor_2023) |> 
  summarize(
    no_hh_2023 = sum(weights, na.rm = TRUE),
    no_pp_2023 = sum(weights * hhsize, na.rm = TRUE)
    ) |> 
  ungroup()

# Baseline
poor_2030_baseline <- ca_microsim01 |>
...
poor_2050_baseline <- ca_microsim01 |>
...
# Dry/Hot
poor_2030_dryhot <- ca_microsim01 |>
...
poor_2050_dryhot <- ca_microsim01 |>
...
# Wet/Warm
poor_2030_wetwarm <- ca_microsim01 |>
...
poor_2050_wetwarm <- ca_microsim01 |>
...
#   summarize(
#     no_hh_2030_baseline_labprod = sum(hh_weight_2030_baseline, na.rm = TRUE),
#     no_pp_2030_baseline_labprod = sum(hh_weight_2030_baseline * hhsize, na.rm = TRUE),
#     ) |> 
#   ungroup()
# 
...
#   summarize(
#     no_hh_2050_baseline_labprod = sum(hh_weight_2050_baseline, na.rm = TRUE),
#     no_pp_2050_baseline_labprod = sum(hh_weight_2050_baseline * hhsize, na.rm = TRUE),
#     ) |> 
#   ungroup()
...
```{r}
poor_2030_labprod_region <- ca_microsim_cc02 |> 
  group_by(RegNo,poor_2030_baseline_labprod) |>  
  summarize(
    # no_hh_2023 = sum(weight_2023, na.rm = TRUE),
    no_pp_2030_labprod = sum(weight_2030_baseline * hhsize, na.rm = TRUE)
    ) |> 
  pivot_wider(
    id_cols = RegNo,
    names_from = poor_2030_baseline_labprod,
    values_from = c(
      # no_hh_2023,
      no_pp_2030_labprod)
  ) |> 
...
poor_2030_baseline_region <- ca_microsim_cc02 |> 
