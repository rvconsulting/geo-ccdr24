...
And we plot it for reference (see @fig-map-adm1-2). 
...
tmap::tm_shape(adm1)+
  tmap::tm_fill("region", legend.show = FALSE, palette = "Set1") +
  tmap::tm_text("region", size = 0.65, auto.placement = T, col = "black")+
...
head(
  ...
  c("UID", "MemberNo", "SAM3_job1", "SAM3_job2")]) |> 
  gt()
```
...
First we find out who has a job but no economic sector.

```{r}
pp_emstatus <- pp_emstatus |> 
  mutate(
    # Annual income
    annual_labor_income_primary = flab_job1 * 4,
    annual_labor_income_secondary = flab_job2 * 4,
    annual_income_surplus = fsurp * 4
  ) |>
  mutate(
    # Annual labor income in thousand GEL
    annual_labor_total =
      (coalesce(
        annual_labor_income_primary, 0) +
       coalesce(
        annual_labor_income_secondary,0)
      )/1000)
```
...
pp_emstatus <- pp_emstatus |>
  ...
  mutate(
    ...
    other_sector = 
      if_else(employed == 1 & !is.na(SAM3_job1), SAM3_job1, NA_real_)
  ) |>
  ...
  mutate(
    ...
    SAM3_job1 = 
      if_else(is.na(SAM3_job1) & employed == 1, other_sector, SAM3_job1)
  ) |>
...
#| eval: false
pp_microsim <- pp_microsim |>
  left_join(poverty_designations, by = "UID")
```
...
#| eval: false
incomes <- ic |> 
  select(UID, inc1, inc2, inc3, inc4, inc5, inc6, inc7, inc8)
```
...
#| eval: false
pp_microsim <- pp_microsim |>
  select(UID, person_id, industry, salaried,
